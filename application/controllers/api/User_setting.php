...
class User_setting extends REST_Controller {
    ...
    public function __construct() { 
        ...
        parent::__construct();
        $this->load->model('api/User_setting_model','model'); //load user model
    }
        ...
        if(!empty($old_number) && !empty($new_number) && !empty($user_id)) {
            ...
            if($list) {
                ...
                if($insert) //check if the user data inserted
                {
                    ...
                    <child>
                    <user>IQRATY</user>
                    <key>d8f416bbbdXX</key>
                    <mobile>'.$new_number.'</mobile>
                    <message>Your OTP Number is:'.$key_word.'</message>
                    <accusage>1</accusage>
                    <senderid>SFTLNK</senderid>
                    </child>

                    </parent>';

                    $URL = "sms.otwodesigns.com/submitsms.jsp?"; 
